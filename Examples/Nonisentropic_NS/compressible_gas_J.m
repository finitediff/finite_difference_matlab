...
function jacobian = compressible_gas_J(matrices, time, K, H, parameters)
    V = matrices{1};
    U = matrices{2};
    e = matrices{3};
    cnu = parameters.cnu;
    ...
    jacobianDimensions = length(matrices)*(length(matrices{1})-2);
    jacobian = zeros(jacobianDimensions);
    quadrantDimensions = length(matrices{1})-2;
    n = time -1;
